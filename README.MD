...
## 问：`backend/routers/ping.py` 里面写了好多测试的接口，项目上线会不会造成安全问题？

> 不会。
>
> `backend/routers/__init__.py` 中第 21-22 行，ping.py中的接口，只有在`settings.enable_test_router==True` 时才有效
>
...
> 首先很感谢您对我的支持！但是，此仓库是个镜像仓库，不接受pr,有事情可以提issue！